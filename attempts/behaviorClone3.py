...
## Move preprocessing into the Lamda layer as in the original comma.ai model
##
...
fileModelJSON = 'model3.json'
fileWeights = 'model3.h5'
...
def batchgen(X, Y):
    while 1:
        for i in range(len(X)):
            ...
            y = np.array([[y]])
            image = image.reshape(1, ch, img_rows, img_cols)
...
model.add(Lambda(lambda x: x/127.5 - 1.,
          ...
          output_shape=(ch, img_rows, img_cols)))
