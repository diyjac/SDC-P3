...
SDC-P3 is a program written in Python to train a regression Keras CNN model with a Tensorflow backend to steer a simulated car in Udacity's Open Sourced Self-Driving Car Simulator: [https://github.com/udacity/self-driving-car-sim](https://github.com/udacity/self-driving-car-sim)

First you will use the simulator to record your driving on track 1.  Then we will train our model with the recorded set of images and steering angles and save the trained model.  Then we start up the car simulator again and put it into autonomous mode have have our trained model drive the simulator car!
